...
export const TimerItem: React.FC<TimerItemProps> = ({ timer }) => {
  ...
  useEffect(() => {
    if (timer.isRunning) {
      intervalRef.current = setInterval(() => {
        ...
        console.log(' timer.remainingTime', timer.remainingTime)

        if (hasEndedRef && (timer.remainingTime == 1)) {
                  hasEndedRef.current = true;
                  timerAudio.play().catch('something went wrong');
                  ...
                  const toastId = toast.success(`Timer "${timer.title}" has ended!`, {
                    ...
                    action: {
                      ...
                      onClick: () => {
                        timerAudio.stop();
                        clearInterval(audioInterval); // Stop playing audio on dismiss
                      },
                    },
                  });
          
                  // Automatically stop audio when the toast finishes (after 5 seconds)
                  setTimeout(() => {
                    timerAudio.stop();
  ...
  }, [timer.isRunning,  timer.id, timer.remainingTime, timer.title, timerAudio, hasEndedRef]);
